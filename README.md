...
獅尾惡腿黑體基於[思源黑體](https://github.com/adobe-fonts/source-han-sans)的筆觸長箭頭改造，更加簡明現代化的字體。支援简体中文、繁體中文、韓文與日文；可以免費商用，歡迎大家自由應用、自由優化、自由改作！

獅尾惡腿黑體的特色是：筆觸起頭與收尾有三角形箭頭。

「獅尾惡魔黑體」與「獅尾惡腿黑體」差別在有沒有拔腳，「拔腳」就是會有較少的筆畫，例如移除「口」字的左右下角會多出的短線條。有拔腳的字會較圓，但是會造成有些字因直接去掉腿後左右不平衡、左右不對襯的「懸空」問題。改使用「腿」系列字體就不會有這個問題。

![字體預覽](https://github.com/max32002/swei-devil-leg/raw/master/preview/welcome.png)

「獅尾惡腿黑體」設計上針對筆畫粗細支援7種字重，所以不論大標、小標、簡單的段落排版都能清晰辨識：
![字體比較預覽](https://github.com/max32002/swei-devil-leg/raw/master/preview/compare_style.png)
...
## 更新日誌
[點擊此處](https://github.com/max32002/swei-devil-leg/blob/master/change_log.md) 查看更新記錄。
...
```
@font-face {
  font-family: SweiDevilLegCJKtc-Regular;
  src: url(https://cdn.jsdelivr.net/gh/max32002/swei-devil-leg@2.0/WebFont/CJK%20TC/SweiDevilLegCJKtc-Regular.woff2) format("woff2")
  , url(https://cdn.jsdelivr.net/gh/max32002/swei-devil-leg@2.0/WebFont/CJK%20TC/SweiDevilLegCJKtc-Regular.woff) format("woff");
}
```
...
* 演算黑科技將字體筆觸長箭頭改造，請參考「獅尾惡魔黑體」專案的 /python/ 目錄下的腳本檔案。透過調整程式碼，也許也可以產生出新的有趣字型。目前的程式應該還有很多錯誤或需要再加強的地方。請先把要處理的字型，透過 FontForge 開啟，並另存 FontForge 的專案為資料夾格式(.sfdir)，最後就可以透過Max的 Python 程式去處理產生出來的檔案。
